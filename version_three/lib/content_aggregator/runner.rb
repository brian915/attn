...
module ContentAggregator
  class Runner
    ...
    def run
      config = ContentAggregator::Config.load(@config_path)
      run_id = @now.utc.strftime('%Y%m%dT%H%M%SZ')

      storage = ContentAggregator::Storage.new(config.dig('output', 'base_dir'))
      ...
      begin
        gmail_client = ContentAggregator::GmailClient.new(config['gmail'], logger: @logger, now: @now)
        summarizer = ContentAggregator::Summarizer.new(config['llm'], logger: @logger)
        ranker = ContentAggregator::Ranker.new(config['ranking'], logger: @logger, now: @now)
        ...
        summary_records.each do |summary_record|
        ...
        manifest['paths']['records_dir'] = run_paths['records_dir']
        manifest['paths']['summaries_dir'] = run_paths['summaries_dir']
    ...
    def summarize_messages(new_messages, summarizer, storage, run_paths)
      ...
      new_messages.each do |message|
        ...
        summary_record = {
          ...
          'summary_created_at' => Time.now.utc.iso8601,
          'source_record_path' => File.join(run_paths['records_dir'], "message_#{message['gmail_message_id'].downcase.gsub(/[^a-z0-9]+/, '_')}.json"),
          'summary_text' => summary_data['summary_text'],
          'why_it_matters' => summary_data['why_it_matters'],
          'key_signals' => summary_data['key_signals'],
          'summary_source' => summary_data['summary_source']
        }

        summary_file_path = storage.write_summary(summary_record, run_paths['summaries_dir'])
        summary_record['summary_file_path'] = summary_file_path
    ...
    def build_initial_manifest(run_id, config, run_paths)
      {
        ...
        'finished_at' => nil,
        'status' => 'running',
        'config_path' => config.dig('meta', 'resolved_config_path'),
        'paths' => {
          'run_dir' => run_paths['run_dir'],
          'manifest_path' => run_paths['manifest_path'],
          'processed_ids_path' => run_paths['processed_ids_path']
        },
        'counts' => {
          'fetched' => 0,
          'new' => 0,
